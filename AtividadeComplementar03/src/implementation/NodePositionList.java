...
public class NodePositionList<E> implements PositionList<E> {
	...
	public void addAfter(Position<E> p, E e) throws InvalidPositionException {
		...
		for (Iterator<E> it = iterator(); it.hasNext();) {
			if (it.next() == p.element())
				aux = true;
		}
		
		if (!aux)
			throw new InvalidPositionException("This position don't belong to this NodeList");
	...
	public NodePositionList<E> inverteNodePositionList() {
		
		NodePositionList<E> node_list_aux = new NodePositionList<E>();
		
		while (numElts > 0) {
			DNode<E> top = trailer.getPrev();
												
			if (node_list_aux.isEmpty()) 
				node_list_aux.addFirst(top.element());
			else {
				node_list_aux.addLast(top.element());
			}
			
			this.remove(this.last());
		}
		
		return node_list_aux;
	}
